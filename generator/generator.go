...
func (g *Generator) generateFile(protoFile *descriptorpb.FileDescriptorProto) (*pluginpb.CodeGeneratorResponse_File, error) {
	...
	b.P("// SPDX-License-Identifier: Apache-2.0")
	...
	for _, message := range protoFile.GetMessageType() {
		err := g.generateMessage(message, packageName, b)
		if err != nil {
			return nil, err
		}
	}
	...
	if g.helperMessages[packageName] != nil {
		...
		for _, helperMessage := range g.helperMessages[packageName] {
			err := g.generateMessage(helperMessage, packageName, b)
			if err != nil {
				return nil, err
			}
		}
...
func (g *Generator) dependencyToImportPath(dependency string) string {
	...
	if strings.Contains(dependency, "node_modules/") {
		...
		parts := strings.Split(dependency, "node_modules/")
		if len(parts) > 1 {
			return parts[1] + ".sol"
		}
	}
